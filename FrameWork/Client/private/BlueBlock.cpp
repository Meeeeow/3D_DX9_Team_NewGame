#include "stdafx.h"
#include "..\public\BlueBlock.h"
#include "Player.h"
#include "Collision.h"

CBlueBlock::CBlueBlock(_pDevice pGraphicDevice)
	: CTileCube(pGraphicDevice)
{
}

CBlueBlock::CBlueBlock(const CBlueBlock & rhs)
	: CTileCube(rhs)
{
}

HRESULT CBlueBlock::NativeConstruct_Prototype()
{
	if (FAILED(__super::NativeConstruct_Prototype()))
		MSG_FAIL("CBlueBlock::NativeConstruct_Prototype - __super");

	return S_OK;
}

HRESULT CBlueBlock::NativeConstruct(void * pArg)
{
	if (FAILED(__super::NativeConstruct(pArg)))
		MSG_FAIL("CBlueBlock::NativeConstruct - __super");
...
HRESULT CBlueBlock::SetUp_Component()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);

	if (FAILED(Add_Component(LEVEL_STATIC, TEXT("PrototypeTransform"), TEXT("ComTransform"), (CComponent**)&m_pTransform)))
		MSG_FAIL("CBlueBlock SetUp_Component - Add_Component m_pTransformCom");

	if (FAILED(Add_Component(LEVEL_STATIC, TEXT("PrototypeRenderer"), TEXT("ComRenderer"), (CComponent**)&m_pRenderer)))
		MSG_FAIL("CBlueBlock SetUp_Component - Add_Component m_pRendererCom");

	if (FAILED(Add_Component(LEVEL_STATIC, TEXT("PrototypeFrustum"), TEXT("ComFrustum"), (CComponent**)&m_pFrustum)))
		MSG_FAIL("CBlueBlock SetUp_Component - Add_Component m_pFrustumCom");

	if (FAILED(Add_Component(LEVEL_STATIC, TEXT("PrototypeVIBufferCube"), TEXT("ComVIBuffer"), (CComponent**)&m_pVIBuffer)))
		MSG_FAIL("CBlueBlock SetUp_Component - Add_Component m_pVIBufferCom");

	if (FAILED(Add_Component(LEVEL_GAMEPLAY, TEXT("PrototypeTextureBlueBlock"), TEXT("ComTexture"), (CComponent**)&m_pTexture)))
		MSG_FAIL("CBlueBlock SetUp_Component - Add_Component m_pTextureCom");
	...
	m_pTransform->SetUp_State(CTransform::STATE_POS, _vec3(28.f, 3.f, 20.f));
...
HRESULT CBlueBlock::SetUp_Component(void * pArg)
{
	...
	if (FAILED(Add_Component(LEVEL_BOSS_CIRCUS, TEXT("PrototypeTextureBlueBlock"), TEXT("ComTexture"), (CComponent**)&m_pTexture)))
		MSG_FAIL("CBlueBlock SetUp_Component - Add_Component m_pTextureCom");
...
void CBlueBlock::BlueBlock_Debuff(CPlayer * pPlayerObject)
{
	CTransform* pPlayerTransform = (CTransform*)pPlayerObject->Get_Component(pTagComTransform);

	pPlayerTransform->SetUp_DebuffSpeed(4.5f);
	pPlayerTransform->SetUp_DebuffTime(0.1f);
}

CGameObject * CBlueBlock::Clone(void * pArg)
{
	CBlueBlock* pInstance = new CBlueBlock(*this);
	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSGBOX("CBlueBlock::Clone Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CBlueBlock::Collision(CGameObject * pColObj)
{
	if (!m_bCollision)
		return;
	...
	if (eCurrentCamera == CAMERA_BACK)
	{
		if (CCollision::CubeCollision(pPlayerObject, (_plane*)m_pBoxCollider->Get_PlaneDirArr(), &vResultPos, &bOntheCube, &bUndertheCube, _vec3(0.f, 0.5f, 0.f)))
		{
			...
			BlueBlock_Debuff(pPlayerObject);
		}
	}
...
CBlueBlock * CBlueBlock::Create(_pDevice pGraphicDevice)
{
	CBlueBlock* pInstance = new CBlueBlock(pGraphicDevice);
	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("CBlueBlock::Create Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CBlueBlock::Free()
{
	__super::Free();
}
